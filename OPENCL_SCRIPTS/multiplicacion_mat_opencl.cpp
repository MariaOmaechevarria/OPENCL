'''
ARCHIVO QUE EJECUTA EL KERNEL DE MULTIPLICACIÓN DE MATRICES EN OPENCL
'''
...
int mult_matrices_basica(cl_device_type device_type) {
    ...
    // Definir tama�o de las matrices
    cl_int dim = 1024;
    ...
    // Definir el tama�o global y local
    size_t global_size[2] = { dim, dim };
    size_t local_size[2] = { 8, 8 };

    // Crear un evento para medir el tiempo de ejecuci�n del kernel
    cl_event event;
    ...
    // Medir el tiempo de ejecuci�n del kernel
    ...
    printf("Tiempo de ejecuci�n del kernel: %.3f s\n", execution_time);
