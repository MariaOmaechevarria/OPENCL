...
def filtrar_extraer(lista_df):
    ...
    for df in lista_df:
        ...
        df_dimensions = df_filtered.columns[1:].astype(int)
        df_times = df_filtered.iloc[0, 1:].values

        # Filtrar dimensiones desde 4 hasta 2048
        df_valid_indices = (df_dimensions >= 4) & (df_dimensions <= 2048)
        df_dimensions = df_dimensions[df_valid_indices]
        df_times = df_times[df_valid_indices]
