...
__kernel void MatrixMul_Local_Tiles(int N,__global float* A, __global float* B, __global float* C, __local float* local_A, __local float* local_B) {
    ...
    for (int phase = 0; phase < N / TILE_WIDTH; phase++) {
        ...
        local_A[ty * TILE_WIDTH + local_id_x] = A[i * N + (phase * TILE_WIDTH + local_id_x)];
        local_B[ty * TILE_WIDTH + local_id_x] = B[(phase * TILE_WIDTH + local_id_y) * N + j];
...
MatrixMul_kernel_local_A="""
    __kernel void MatrixMul_kernel_local_A(int dim,__global float *A,__global float *B,__global float *C,__local float *local_A)
{
 // Obtener la información de los índices
...
// Tamaño grupo trabajo
 ...
 for(int i=0;i< dim;++i)
 {
         result += local_A[i]*B[i*dim + iCol];
 }
 //Almacenar resultado
 C[global_id_y*dim + iCol] = result;
}
"""
