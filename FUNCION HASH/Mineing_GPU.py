...
__kernel void kernel_mining(__global const char* data, 
    ...
    if (hash_value_high < target_high || (hash_value_high == target_high && hash_value_low < target_low)) {
        if (atomic_cmpxchg(found, 0, 1) == 0) { // Marca como encontrado de manera atómica si no ha sido marcado aún
            *new_nonce = nonce; // Guarda el nonce que cumplió la condición
            
            for (int i = 0; i < 32; i++) { // Copiar el hash ganador
...
def mining_GPU(kernel_name, kernel_code, device_type, header, target):
...
for target in targets:
    ...
    exec_time, result_nonce, hash = mining_GPU(kernel_name, kernel_mining, device_type, header_bytes, target)
    print(hash)
